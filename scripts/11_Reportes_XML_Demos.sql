...
BEGIN
    ...
    ;WITH F AS (
        SELECT *
        FROM Reportes.VW_FlujoCajaSemanal
...
BEGIN
    ...
    IF @FechaDesde IS NULL OR @FechaHasta IS NULL
    BEGIN
        RAISERROR('Indique @FechaDesde y @FechaHasta',16,1);
        RETURN;
    END;

    -- Fusible anti-reentrada en esta sesión
    IF TRY_CAST(SESSION_CONTEXT(N'RecaudProcedencia_Run') AS bit) = 1
        RETURN;

    EXEC sys.sp_set_session_context @key = N'RecaudProcedencia_Run', @value = 1;
    ...
    END TRY
    BEGIN CATCH
        EXEC sys.sp_set_session_context @key = N'RecaudProcedencia_Run', @value = NULL;
        THROW;
    END CATCH;
